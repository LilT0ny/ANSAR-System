    ...
    return (
        <div className="space-y-8">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-500">
                <div>
                    <h1 className="text-2xl md:text-3xl font-serif font-bold text-gray-800">Salud Financiera</h1>
                    <p className="text-secondary mt-1 text-sm md:text-base">Analiza ingresos, egresos y la rentabilidad de tu clínica.</p>
                </div>

                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    {/* Date Filters */}
                    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 md:px-4 py-2.5 shadow-sm overflow-x-auto no-scrollbar">
                        <Filter size={16} className="text-gray-400 shrink-0" />
                        <input
                            type="date"
                            value={dateFrom}
                            onChange={(e) => setDateFrom(e.target.value)}
                            className="text-xs md:text-sm outline-none text-gray-600 bg-transparent shrink-0"
                        />
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="flex-1 sm:flex-none bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm outline-none focus:ring-2 focus:ring-primary/30"
                        >
                        ...
                        <button
                            ...
                            className="flex-1 sm:flex-none bg-primary hover:bg-green-600 text-white px-4 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 text-sm transition-all shadow-md hover:shadow-lg active:scale-95"
                        >
                            <Plus size={16} /> <span className="hidden xs:inline">Registrar Egreso</span><span className="xs:hidden">Egreso</span>
                        </button>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {kpis.map((kpi, i) => (
                    <div key={i} className={`bg-white p-5 md:p-6 rounded-2xl shadow-sm border ${kpi.border} transition-all hover:shadow-md duration-300 group`}>
                        <div className="flex items-center justify-between mb-4">
                            <div className={`${kpi.bg} p-3 rounded-xl transform transition-transform group-hover:scale-110`}>
                                <kpi.icon size={22} className={kpi.color} />
                            </div>
                            {kpi.trendUp !== null && (
                                <div className={`flex items-center gap-1 text-[10px] md:text-xs font-bold px-2 py-1 rounded-full ${kpi.trendUp ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
                                    {kpi.trendUp ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
                                    {kpi.trend}
                                </div>
                            )}
                            {kpi.trendUp === null && (
                                <span className="text-[10px] md:text-xs font-bold px-2 py-1 rounded-full bg-amber-50 text-amber-600">{kpi.trend}</span>
                            )}
                        </div>
                        <p className="text-xs md:text-sm font-medium text-gray-500 font-sans">{kpi.label}</p>
                        <h3 className="text-xl md:text-2xl font-bold text-gray-800 mt-1 font-serif">{kpi.value}</h3>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-base md:text-lg font-serif font-bold text-gray-800">Ingresos vs. Egresos</h3>
                        <span className="text-[10px] font-bold text-gray-400 bg-gray-50 px-3 py-1.5 rounded-full uppercase tracking-wider">{selectedYear}</span>
                    </div>
                    <div className="h-72">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 md:p-6 border-b border-gray-100 gap-4">
